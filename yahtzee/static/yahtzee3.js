      ...
      .then(scores => {
        if (scores.total !== null) {
          if (this.currentPlayer === 1) {
            ...
            else if (this.currentRoundIndex === 5 && this.totalScorePlayer3) {
              this.currentRoundIndex += 1;
          }
        }
        ...
        if (this.currentRoundIndex >= roundDescriptions.length) {
          if (this.totalScore > this.totalScorePlayer2 && this.totalScore > this.totalScorePlayer3) {
            alert(`Game Over! ${this.player1Name} is the winner with ${this.totalScore} points!`);
          } else if (this.totalScorePlayer2 > this.totalScore && this.totalScorePlayer2 > this.totalScorePlayer3) {
            alert(`Game Over! ${this.player2Name} is the winner with ${this.totalScorePlayer2} points!`);
          } else if (this.totalScorePlayer3 > this.totalScore && this.totalScorePlayer3 > this.totalScorePlayer2) {
            alert(`Game Over! ${this.player3Name} is the winner with ${this.totalScorePlayer3} points!`);
          } else {
...
diceController.run();
  ...
  scoreRows.forEach((row, index) => {
    row.childNodes[1].textContent = diceController.roundScores[index] || "";
    row.childNodes[2].textContent = diceController.roundScoresPlayer2[index] || "";
    row.childNodes[3].textContent = diceController.roundScoresPlayer3[index] || "";
    row.childNodes[1].classList.remove("current-round");
    row.childNodes[2].classList.remove("current-round");
    row.childNodes[3].classList.remove("current-round");
    // Check if it is still the same round and update the green marker
    if (index === diceController.currentRoundIndex) {
      if (diceController.currentPlayer === 1) {
        row.childNodes[1].classList.add("current-round");
      } else if (diceController.currentPlayer === 2) {
        row.childNodes[2].classList.add("current-round");
      } else {
        row.childNodes[3].classList.add("current-round");
      }
